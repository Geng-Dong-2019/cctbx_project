...
class LocalRefiner(BaseRefiner):
    ...
    def __init__(self, n_total_params, n_local_params, local_idx_start,
        ...
        self.ROI_GAINMAPS = None
        if shot_roi_gainmaps is not None:
            self.ROI_GAINMAPS = self._check_keys(shot_roi_gainmaps)
        if verbose:
    ...
    def print(self, *args, **kwargs):
        print(*args, **kwargs, end=self.print_end)
    ...
    def _setup(self):
        ...
        if self.I_AM_ROOT:
            self.print("--1 Setting up per shot parameters", flush=True)
    ...
    def _update_eta(self):
        if self.update_eta or self.refine_eta:  # TODO think of a better interface
            ...
            self.print("updating ETA as %f deg sampled from %d blocks" % (eta_val, self.D.mosaic_domains))
        ...
        if self.refine_ncells:
            ...
            for i_ncell in range(self.n_ncells_param):
                if self.use_Ncells_range:
                    ...
                    xval = self.Xall[xpos]
                    d = self.m_dI_dtheta[i_ncell]
                    d = self.Ncells_params[self._i_shot][i_ncell].get_deriv(xval, d)  # update the derivative according to the reparameterization
                    d2 = 0
                else:
                ...
                if self.calc_curvatures:
                    if self.use_Ncells_range:
                        raise NotImplementedError("Cannot refine with curvatures when using ncells ranges")
                    self.curv[xpos] += self._curv_accumulate(d, d2)
    ...
    def print_step_grads(self):
        ...
        self.print(
            "%s\n\t%s, F=%2.7g, |G|=%2.7g, eps*|X|=%2.7g,%s R1=%2.7g (R1 at start=%2.7g), Fcell negative_models=%d, Neg. Curv.: %d/%d on shots=%s\n"
    ...
    def _combine_data_to_save(self):
        ...
        for i_shot in my_shots:
            ...
            self.data_to_send.append(
                (proc_h5_fname, proc_h5_idx, proc_bbox_idx, exp_name, crystal_scale, eta, Amat_refined, ncells_val, bgplane,
    ...
    def get_lbfgs_x_array_as_dataframe(self):
        ...
        if self.I_AM_ROOT:
            ...
            fnames, shot_idx, bbox_idx, exp_name, xtal_scales, eta, Amats, ncells_vals, bgplanes, image_corr, init_img_corr, \
    ...
    def _update_sigma_r_if_possible(self):
        if self.ROI_GAINMAPS is not None and self.LOW_RMS is not None and self.MED_RMS is not None and self.HIGH_RMS is not None:
            x1, x2, y1, y2 = self.ROIS[self._i_shot][self._i_spot]
            gain_mode = self.ROI_GAINMAPS[self._i_shot][self._i_spot]
            assert set(gain_mode.ravel()).issubset({0, 1, 3})
            roi_shape = y2-y1, x2-x1
            pedestalRMS = NP_ZEROS(roi_shape)
            gain_mode_is_low = gain_mode == 0
            gain_mode_is_med = gain_mode == 1
            gain_mode_is_high = gain_mode == 3

            pedestalRMS[gain_mode_is_low] = self.LOW_RMS[self._panel_id, y1:y2, x1:x2][gain_mode_is_low]
            pedestalRMS[gain_mode_is_med] = self.MED_RMS[self._panel_id, y1:y2, x1:x2][gain_mode_is_med]
            pedestalRMS[gain_mode_is_high] = self.HIGH_RMS[self._panel_id, y1:y2, x1:x2][gain_mode_is_high]
            assert pedestalRMS.min() >= 0
            return pedestalRMS.ravel()
        else:
            return self.sigma_r
