...
class LocalRefinerLauncher:
    ...
    def __init__(self, params):
        ...
        self.symbol = None
        self.LOW_RMS = self.HIGH_RMS = self.MED_RMS = None
    ...
    def launch_refiner(self, refls, expt, miller_data=None):
        ...
        self.shot_roi_gainmaps = {0: shot_data.roi_gainmaps}
    ...
    def _launch(self, n_local_unknowns, n_global_unknowns, local_idx_start, global_idx_start):
        ...
        for i_trial in range(n_trials*nmacro):
            ...
            self.RUC.LOW_RMS = self.LOW_RMS
            self.RUC.HIGH_RMS = self.HIGH_RMS
            self.RUC.MED_RMS = self.MED_RMS
    ...
    def load_roi_data(self, refls, expt):
        ...
        gain_data = None
        if self.params.refiner.gainhack is not None:
            gain_data = utils.gain_data_from_expt(expt)
    ...
    def _init_refiner(self, n_local_unknowns, n_global_unknowns, local_idx_start, global_idx_start):
        ...
        RUC = self._Refiner(
            ...
            shot_roi_gainmaps=self.shot_roi_gainmaps,
    ...
    def load_gainRMS_images(self):
        if self.params.refiner.gainhack is not None:
            print("Loading gainRMS images")
            import dxtbx
            loader = dxtbx.load(self.params.refiner.gainhack)
            print("Image indices 0,1,2 correspond to LOW(0), MED(1), HIGH(3) gain modes, repsectively")
            print("\tLOW")
            self.LOW_RMS = np.array([d.as_numpy_array() for d in loader.get_raw_data(2)])
            print("\tMED")
            self.MED_RMS = np.array([d.as_numpy_array() for d in loader.get_raw_data(1)])
            print("\tHIGH")
            self.HIGH_RMS = np.array([d.as_numpy_array() for d in loader.get_raw_data(0)])
